...
export class PaymentController {
  static async addPayment(req: Request, res: Response) {
    try {
      ...
      if (!service || !email || !cardholder || !cardnumber || !exp_month || !exp_year || !cvv || !amount || !currency) {
        return res.status(400).json({ message: 'Todos los campos son obligatorios.' });
      }

      await PaymentModel.savePayment({ service, email, cardholder, cardnumber, exp_month, exp_year, cvv, amount, currency });
